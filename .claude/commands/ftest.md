...
1. **테스트 목적**: 파일 첫 줄에 `// Test: <목적>` 주석으로 이 테스트가 무엇을 검증하는지 기술
2. **파일명**: `tests/{feature-name}.flt` (kebab-case)
3. **Command**: 실제 실행 가능한 명령 (Step 0에서 감지한 실행 방법 사용)
4. **Input**: 기능을 트리거하는 최소한의 입력
5. **Output**: 실제로 실행하여 확인한 정확한 기대 출력
6. **ExitCode**: 에러 케이스면 0이 아닌 코드
7. **Stderr**: 에러 메시지가 예상되면 추가
8. **Timeout**: 장시간 실행 가능하면 추가
